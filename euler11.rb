...
#make numbers
20.times do |j|
	grid[j].map!  {|i| i = i.to_i}
end
#check products going across
max_prod = 0
row = 0
col = 0
j = 0
while j<20
	...
	i = 0
	while i <17
		prod = x[i]*x[i+1]*x[i+2]*x[i+3] 
		if prod > max_prod
			max_prod = prod
			col = i
			row = j
			puts "#{max_prod} #{col} #{row}"
			puts "#{x[i]} #{x[i+1]} #{x[i+2]} #{x[i+3]}"
		end
		i += 1
	end
	j += 1
...
while i < 20
	...
	while j < 17
		...
		if prod > max_prod
			max_prod = prod
			puts "#{max_prod} #{i} #{j}"
			puts "#{grid[j][i]} #{grid[j+1][i]} #{grid[j+2][i]} #{grid[j+3][i]}"
		end
		j += 1
	end
	i += 1
...
while j < 17
	...
	while i < 17
		...
		if prod > cross
			cross = prod
			puts "#{cross} #{i} #{j}"
			puts "#{grid[j][i]} #{grid[j+1][i+1]} #{grid[j+2][i+2]} #{grid[j+3][i+3]}"
		end
		i += 1
	end
	j += 1
